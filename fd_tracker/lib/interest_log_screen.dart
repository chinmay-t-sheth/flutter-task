...
class _InterestLogScreenState extends State<InterestLogScreen> {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
                'If you reinvest the maturity amount of ${_formatCurrency(maturityAmount)}:'),
            const SizedBox(height: 16),
            Text('At a rate of ${newRate.toStringAsFixed(2)}%'),
            Text('For a tenure of $tenureMonths months'),
            const Divider(height: 20),
            Text(
              'The new estimated maturity value would be:',
              style: TextStyle(color: Colors.grey[600]),
            ),
            const SizedBox(height: 4),
            Text(
              _formatCurrency(simulatedAmount),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.fd.title),
        centerTitle: true,
      ),
      body: Consumer<FDProvider>(
        builder: (context, fdProvider, child) {
          final logs = fdProvider.getLogsForFD(widget.fd.id!);
          final upcomingInterest = fdProvider.upcomingInterest(widget.fd.id!);
          final totalInterestReceived =
          fdProvider.getTotalInterestForFD(widget.fd.id!);
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      _buildSummaryItem(
                          'Total Received',
                          _formatCurrency(totalInterestReceived),
                          Icons.check_circle,
                          Colors.green),
                      _buildSummaryItem(
                          'Next Expected',
                          _formatCurrency(upcomingInterest),
                          Icons.schedule,
                          Colors.orange),
                    ],
                  ),
